...
class FileHandler implements \SessionHandlerInterface
{
     ...
     */
    public function __construct(array $options, $lifetime = 0, FileManager $file = null)
    {
    ...
    {
        foreach ($this->file->getFiles($this->directory) as $filename)
        {
    ...
    {
        return $this->file->exists($this->directory . '/' . $session_id)
            ? $this->file->read($this->directory . '/' . $session_id)
            : false;
    }
    ...
    public function write($session_id, $session_data)
    {
        try
        {
            return $this->file->write($this->directory . '/' . $session_id, $session_data);
        }
