...
final class BootloadConfigTest extends InitializerTestCase
{
    ...
    {
        $result = \iterator_to_array($this->initializer->init([
            BootloaderA::class => static fn (): \Spiral\Boot\Attribute\BootloadConfig => new BootloadConfig(args: ['a' => 'b']),
        ]));
    ...
    public function testCallableConfigWithArguments(): void
    {
        ...
        $result = \iterator_to_array($this->initializer->init([
            BootloaderA::class => static fn (AppEnvironment $env): \Spiral\Boot\Attribute\BootloadConfig => new BootloadConfig(enabled: $env->isLocal()),
        ]));
        $this->assertEquals([], $result);

        $result = \iterator_to_array($this->initializer->init([
            BootloaderA::class => static fn (AppEnvironment $env): \Spiral\Boot\Attribute\BootloadConfig => new BootloadConfig(enabled: $env->isProduction()),
        ]));
