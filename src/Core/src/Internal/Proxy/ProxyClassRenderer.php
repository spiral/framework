...
final class ProxyClassRenderer
{
    ...
    private static function cutDefaultValue(\ReflectionParameter $param): string
    {
        ...
        $value = \trim(\substr($string, \strpos($string, '=') + 1, -1));

        return self::normalizeStringLiterals($value);
    }

    private static function normalizeStringLiterals(string $expression): string
    {
        $result = '';
        foreach (\token_get_all('<?php ' . $expression) as $token) {
            if (\is_string($token)) {
                $result .= $token;
                continue;
            }
            ...
            $literal = $token[1];
            $result .= $token[0] === \T_CONSTANT_ENCAPSED_STRING && $literal[0] === '"'
                ? self::exportString(self::decodeDoubleQuotedString(\substr($literal, 1, -1)))
                : $literal;
        }
    ...
    private static function decodeDoubleQuotedString(string $value): string
    {
        $result = '';
        $length = \strlen($value);
        for ($offset = 0; $offset < $length; ++$offset) {
            ...
            if ($escaped === 'x' && $offset + 1 < $length && \str_contains('0123456789abcdefABCDEF', $value[$offset + 1])) {
                $hex = $value[++$offset];
                if ($offset + 1 < $length && \str_contains('0123456789abcdefABCDEF', $value[$offset + 1])) {
                    $hex .= $value[++$offset];
                }
